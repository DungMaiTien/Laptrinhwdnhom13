       ...
       public void DocFile(string path)
        {
            ...
            {
                System.Windows.Forms.MessageBox.Show("Trò chơi thiếu dữ kiện tài nguyên");
            }
        }
        public void draw()
        {
            for (int i = 0; i < max; i++)
                render.Draw(g, new Point(vc[i].X*10,vc[i].Y*10), Pens.LightCyan, Brushes.MediumVioletRed);
